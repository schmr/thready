...
typedef enum {
        ...
        /* trying to pop wrong type of mode. popping array in object mode, vice
           versa */
...
typedef int (*json_parser_callback)(void* userdata,
                                    int type,
                                    const char* data,
                                    uint32_t length);
typedef int (*json_printer_callback)(void* userdata,
                                     const char* s,
                                     uint32_t length);
 ...
 * int, float, key, string need to be followed by a pointer to char and then a
 * length. if the length argument is -1, then the strlen function will use on
 * the string argument. the function call should always be terminated by -1 */
int json_print_args(json_printer*,
...
/** callback from the parser helper callback to append a value to an object or
 * array value append(parent, key, key_length, val); */
 ...
 * use of 3 callbacks where the user can choose the representation of the JSON
 * values */
...
/** helper to parser callback that arrange parsing events into comprehensive
 * JSON data structure */
