...
#define LIST_MAXSIZE 30 /* max. number of elements unrolled into one node */
#define LIST_LIMIT                                                 \
        15 /* this many or fewer number in a node => attempt merge \
            */
...
bool selist_find(struct selist** qlp,
                 int* index,
                 const void* value,
...
bool selist_contains(selist* ql,
                     const void* value,
...
void* selist_get(const selist* ql, int i) {
        assert(ql);
        return (i < ql->num_elements)
                   ? ql->elements[i]
                   : selist_get(ql->next, i - ql->num_elements);
}
...
int selist_delete(selist** qlp, int i) {
        ...
        if (ql && i >= ql->num_elements) {
                ...
                if (i + 1 < ql->num_elements) {
                        size_t len =
                            (size_t)(ql->num_elements - i - 1) * sizeof(void*);
                ...
                if (ql->num_elements == 0) {
                        *qlp = ql->next;
                        ...
                        selist* qn = ql->next;
                        if (ql->num_elements + qn->num_elements >
                            LIST_MAXSIZE) {
                                size_t len = (size_t)(qn->num_elements - 1) *
                                             sizeof(void*);
                                ql->elements[ql->num_elements] =
                                    qn->elements[0];
                                --qn->num_elements;
                                ++ql->num_elements;
                                ...
                                size_t len =
                                    (size_t)qn->num_elements * sizeof(void*);
                                memcpy(ql->elements + ql->num_elements,
                                       qn->elements, len);
...
int selist_insert(selist** qlp, int i, void* data) {
        ...
        if (ql) {
                if (i >= LIST_MAXSIZE) {
                        return selist_insert(&ql->next, i - ql->num_elements,
                                             data);
                } else if (ql->num_elements < LIST_MAXSIZE &&
                           i <= ql->num_elements) {
                        if (i < ql->num_elements) {
                                size_t len = (size_t)(ql->num_elements - i) *
                                             sizeof(void*);
                        ...
                        if (qn) {
                                ...
                                memcpy(
                                    qn->elements,
                                    ql->elements +
                                        ql->num_elements,  // TODO: V512
                                                           // http://www.viva64.com/en/V512
                                                           // A call of the
                                                           // 'memcpy' function
                                                           // will lead to
                                                           // underflow of the
                        ...
                        if (i <= ql->num_elements) {
                                ...
                                return selist_insert(
                                    &ql->next, i - ql->num_elements, data);
                        }
                }
        } else if (i == 0) {
...
void selist_map_reduce(struct selist* ql,
                       void (*mapfunc)(void* entry, void* data),
                       void (*reducefunc)(void* data, void* result),
                       void* data,
                       void* result) {
        int qi;
...
bool selist_set_insert(struct selist** qlp, void* data, cmp_cb cmp) {
        if (*qlp) {
                ...
                if (cmp == NULL)
                        cmp = cmp_voidptr;
                if (ql->num_elements > 0 &&
                    cmp(ql->elements[ql->num_elements - 1], data) < 0) {
                        if (ql->num_elements == LIST_MAXSIZE ||
                            (ql->next &&
...
bool selist_set_find(struct selist** qlp,
                     int* qip,
                     const void* data,
                     cmp_cb cmp) {
        selist* ql = *qlp;
