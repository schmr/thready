 ...
 */
static int is_argv_end(const struct parg_state* ps,
                       int argc,
                       char* const argv[]) {
 ...
 */
static int match_short(struct parg_state* ps,
                       int argc,
                       char* const argv[],
...
static int match_long(struct parg_state* ps,
        ...
        if (ps->nextchar[len] == '=') {
                ...
                if (longopts[match].has_arg == PARG_NOARG) {
                        ps->optopt =
                            longopts[match].flag ? 0 : longopts[match].val;
                        ps->nextchar = NULL;
                        return optstring[0] == ':' ? ':' : '?';
                } else {
...
int parg_getopt(struct parg_state* ps,
                int argc,
                char* const argv[],
                const char* optstring) {
 ...
 */
static int parg_reorder_simple(int argc,
                               char* argv[],
                               const char* optstring,
                               const struct parg_option* longopts) {
        ...
        do {
                ...
                do {
                        c = parg_getopt_long(&ps, argc, argv, optstring,
                                             longopts, NULL);
                ...
                do {
                        ...
                        for (l = nextind; c != 1 && c != -1;) {
                                ...
                                do {
                                        c = parg_getopt_long(&ps, argc, argv,
                                                             optstring,
                                                             longopts, NULL);
                                } while (ps.nextchar != NULL &&
                                         *ps.nextchar != '\0');
                        }
...
int parg_reorder(int argc,
        ...
        do {
                ...
                c = parg_getopt_long(&ps, argc, argv, optstring, longopts,
                                     NULL);
